...
// src/config.ts
var groqModelInitConfigs = {
  ['llama-3.3-70b-versatile' /* LLAMA_3_3_70B_VERSATILE */]: {
    model: 'llama-3.3-70b-versatile' /* LLAMA_3_3_70B_VERSATILE */,
    temperature: 0,
...
function getDefaultFetch() {
  ...
  throw new Error(
    '`fetch` is not defined as a global; Either pass `fetch` to the client, `new Groq({ fetch })` or polyfill the global, `globalThis.fetch = fetch`'
  );
}
...
var createPathTagFunction = (pathEncoder = encodeURIPath) =>
...
var APIPromise = class _APIPromise extends Promise {
  ...
  _thenUnwrap(transform) {
    return new _APIPromise(
      ...
      async (client, props) => transform(await this.parseResponse(client, props), props)
    );
  }
  /**
...
var Groq = class {
  ...
  buildURL(path2, query, defaultBaseURL) {
    const baseURL =
      (!__classPrivateFieldGet(this, _Groq_instances, 'm', _Groq_baseURLOverridden).call(this) &&
        defaultBaseURL) ||
      this.baseURL;
    const url = isAbsoluteURL(path2)
      ? new URL(path2)
      : new URL(
          baseURL + (baseURL.endsWith('/') && path2.startsWith('/') ? path2.slice(1) : path2)
        );
...
((_a = Groq),
  (_Groq_encoder = /* @__PURE__ */ new WeakMap()),
  (_Groq_instances = /* @__PURE__ */ new WeakSet()),
  (_Groq_baseURLOverridden = function _Groq_baseURLOverridden2() {
...
// src/groq/groq-model.ts
var GroqModel = class _GroqModel {
  constructor(groqModelInitConfig) {
    try {
      _GroqModel.groqClient ??= new Groq();
    } catch (e) {
      throw new Error(
  ...
  async getFinalReview(messages) {
    const completion = await _GroqModel.groqClient.chat.completions.create({
      messages,
    ...
    return {
      ...
      ...parsed,
      totalScore:
        Object.values(parsed.metrics).reduce((acc, curr) => acc + curr.score, 0) /
        Object.values(parsed.metrics).length,
    };
  }
};

// src/groq/groq-models.ts
var GroqModels = class {
  constructor(groqModelInitConfigs2) {
    this.groqModels = /* @__PURE__ */ new Map();
    Object.entries(groqModelInitConfigs2).forEach(([groqModelKey, groqModelInitConfig]) => {
      this.groqModels.set(groqModelKey, new GroqModel(groqModelInitConfig));
    });
  }
  async getFinalReview(messages) {
    return Promise.all(
...
// src/prompts/GROQ_REVIEW_PROMPT.txt
var GROQ_REVIEW_PROMPT_default =
  'You are an SEO content evaluator. Analyze the provided blog article and score it on the E-E-A-T framework used by Google:\n\n- E1 (Experience): Does the author demonstrate first-hand or life experience with the topic?\n- E2 (Expertise): Does the content show deep domain knowledge and subject-matter expertise?\n- A (Authoritativeness): Is the content authoritative? Does it cite credible sources or reference recognized authorities?\n- T (Trustworthiness): Is the content accurate, honest, transparent, and reliable?\n\nScore each metric from 1 to 100 (1 = very poor, 100 = excellent).\n\nRespond ONLY with a valid JSON object in this exact structure \u2014 no markdown fences, no explanation outside the JSON:\n{\n  "metrics": {\n    "E1": { "score": <number 1-100>, "description": "<brief explanation>" },\n    "E2": { "score": <number 1-100>, "description": "<brief explanation>" },\n    "A":  { "score": <number 1-100>, "description": "<brief explanation>" },\n    "T":  { "score": <number 1-100>, "description": "<brief explanation>" }\n  },\n  "summary": "<overall assessment in 2-3 sentences>"\n}\n';

// src/lib.ts
async function getFinalReview(content) {
  const groqModels = new GroqModels(groqModelInitConfigs);
  return groqModels.getFinalReview([
    { role: 'system', content: GROQ_REVIEW_PROMPT_default },
    { role: 'user', content },
  ]);
}
